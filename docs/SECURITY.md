...
- [ ] `CPANEL_API_TOKEN` solo en local (Manage API Tokens); no subir al repo ni al servidor
...
| Deploy | `php artisan deploy:cpanel` vía API token cPanel + Cron; `.env` del servidor no se sobrescribe; sin `migrate:fresh` |
